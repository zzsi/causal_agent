...
# 测试
# input_str = '{{"answer":"no"}}'
# answer = extract_answer(input_str)
# print(f"匹配到的答案是: {answer}")


import json
file_names = ['resGPT.jsonl']
        ...
        if item['question_type'] == types:
            ...
            if types in ["IT","CIT","MULTCIT","CAUSE","Has-Collider","Has-Confounder"]:
                try:
                    ...
                    # print(item['output'])
                    output = json.loads(extract_answer(item['output']).replace('{{','{').replace('}}','}').replace('\'','\"'))
                    # print('output : ',output)
                    if output['answer'] == item['gt']:
                        ...
                        pass
                        # print(output['answer'],item['gt'])
                        # error_items.append(item)
                except:
                    # print(item['output'], item['gt'])
                    # error_items.append(item)
                    continue
            if types in ['ATE']:
                try:
