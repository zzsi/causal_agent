...
class Dataloader():
    ...
    def Get_QR_question_answer_pair(self,line):
        q = line['data_description'] + f"\nfile_name: {line['data_files'][0]}\n Command: {line['question']}"
        # print(line)
        return q,line['answer'],line['data_files'][0],line['meta_data']['question_type']

class CGMemory():
    def __init__(self):
        self.CG = {}
        self.index = 1
    def get(self,name) -> (object,Relationship):
    ...
    def add(self,name,cg) -> (str,Relationship):
    ...
    def Delete(self,name)-> (str,Relationship):
        if name in self.CG.keys():
            del self.CG[name]
