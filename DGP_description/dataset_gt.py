...
to_write = []
name = 'ate'
filename = 'new_ate_question.jsonl'
...
question_CG = {
    ...
    "ATE":{
        "What is the Average Treatment Effect (ATE) of the continuous treatment variable {T} on the outcome {Y} when the treatment changes from {T0} to {T1}?",
        "How much is the Average Treatment Effect (ATE) for a continuous treatment {T} on the outcome variable {Y}, given that {T} shifts from {T0} to {T1}?",
        "Can you provide the calculated Average Treatment Effect (ATE) for {T} on {Y} as {T} moves from {T0} to {T1}?",
        "What is the result of the Average Treatment Effect (ATE) for a continuous treatment variable {T} on {Y}, with {T} changing from {T0} to {T1}?",
        "What is the value of the Average Treatment Effect (ATE) when the treatment {T} transitions from {T0} to {T1} and affects the outcome {Y}?",
        "How much is the computed Average Treatment Effect (ATE) for the treatment variable {T} on the outcome {Y}, given that {T} changes from {T0} to {T1}?",
        "What is the ATE result for the continuous treatment {T} on {Y} when {T} varies from {T0} to {T1}?",
        "Could you calculate the Average Treatment Effect (ATE) for the variable {T} on the outcome {Y}, assuming {T} changes from {T0} to {T1}?",
        "What is the Average Treatment Effect (ATE) value for {T} on {Y} when the treatment changes from {T0} to {T1}?",
        "What is the calculated ATE for a continuous treatment {T} on the outcome {Y}, given that the treatment changes from {T0} to {T1}?"
    }
}
...
for id,line in enumerate(to_write):
    ...
    if Q == "ATE":
        ...
        line["Q"] = Q_content.format(T=interest[0], Y=interest[1],T0=line['T0'],T1=line['T1']) + "The remaining variables can be regarded as confounder : " + ', '.join([i for i in line['variables'] if i not in line['interest']])
