import json
file_names = ['baseline-gpt3.5-ate.jsonl']
...
for types in type_names:
    ...
    for item in to_read:
        if item['question_type'] == types:
            number += 1
            if item['node num'] not in attr_num.keys():
                attr_num[item['node num']] = [0,0]
            attr_num[item['node num']][0] += 1
...
with open('error_items.jsonl','w') as f:
